...
| `202607150001` | EBIOS RM Workshop 2: `risk_sources` (standalone catalog, `category`/`motivation`/`resources`/`activity` all backed by Postgres enums); `risk_origins` (the SR/OV pair — `risk_source_id` `NOT NULL` `ondelete="RESTRICT"` since the row *is* the pairing, mirroring `feared_events.asset_id`'s RESTRICT precedent; `feared_event_id` nullable `ondelete="SET NULL"`, matching `risks`' optional-link shape); `risk_sources` created before `risk_origins` for the same FK-ordering reason `202607130001` created `feared_events` before altering `risks`. Seeds 4 new permission codes and grants them — the sixth instance of the idempotent permission-seeding pattern |
| `202607160001` | EBIOS RM Workshop 3: `ecosystem_parties` (standalone catalog, `category`/`dependency_level`/`cyber_maturity` backed by Postgres enums); `strategic_scenarios` (`risk_origin_id` and `feared_event_id` both `NOT NULL` `ondelete="RESTRICT"` — the row *is* the elaboration of that specific pair targeting that specific feared event; `ecosystem_party_id` nullable `ondelete="SET NULL"`, since not every attack path routes through a third party). `ecosystem_parties` created before `strategic_scenarios` for the same FK-ordering reason every prior EBIOS RM migration has followed. Seeds 4 new permission codes and grants them — the seventh instance of the idempotent permission-seeding pattern |
| `202607170001` | EBIOS RM Workshop 4: `operational_scenarios` (`strategic_scenario_id` `NOT NULL` `ondelete="RESTRICT"` — the row *is* the technical elaboration of that specific strategic scenario; `mitre_technique_ids` stored as a plain `JSON` column, not a Postgres-specific `ARRAY`, so it runs against SQLite in tests too — same reasoning as `KnowledgeBaseDocumentModel.embedding`). Seeds 2 new permission codes and grants them — the eighth instance of the idempotent permission-seeding pattern |
| `202607180001` | EBIOS RM Workshop 5 (the final workshop): `risk_treatments` (`strategic_scenario_id` `NOT NULL` `ondelete="RESTRICT"` — same reasoning as every other EBIOS RM FK chain; deliberately no uniqueness constraint on `strategic_scenario_id`, since re-deciding a treatment means inserting a new row, not updating one — an append-only decision history). Seeds 2 new permission codes and grants them — the ninth instance of the idempotent permission-seeding pattern |
